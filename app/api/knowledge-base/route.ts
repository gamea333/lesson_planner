...
export async function GET(request: NextRequest) {
  try {
    ...
    if (filtersOnly) {
      return NextResponse.json(await getKnowledgeBaseFilters(grade, subject));
    }

    const entries = await listKnowledgeBaseEntries({ grade, subject, search });
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const grade =
          (formData.get(`grade_${file.name}`) as string)?.trim() ||
          processed.grade;
        const subject =
          (formData.get(`subject_${file.name}`) as string)?.trim() ||
          processed.subject;
        const chapter =
          (formData.get(`chapter_${file.name}`) as string)?.trim() ||
          processed.chapter;

        const entry = await insertProcessedPdf({
          ...processed,
          grade: grade || "",
          subject: subject || "",
          chapter: chapter || "",
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updated = await updateKnowledgeBaseEntry(Number(id), {
      grade: grade.trim(),
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await deleteKnowledgeBaseEntry(id);
    if (!deleted) {
      return NextResponse.json({ error: "Entry not found" }, { status: 404 });
    }
